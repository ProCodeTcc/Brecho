...
<script>
    //função ao sair do campo de cep
	$('.txt_cep').blur(function(){
        //resgatando o valor do CEP
        var cep = $('.txt_cep').val();
        
        //aguardando os valores serem preenchidos
        $('.txtlogradouro').val('...');
        $('.txtbairro').val('...');
        $('.txtestado').val('...');
        $('.txtcidade').val('...');
...
<form method="POST" action="../router.php?controller=ClienteFisico&modo=cadastrar" id="fisico">
	...
	<div class="linha_cadastro_usuario">
		<input class="campo_cadastro_usuario" type="email" name="txtEmail" onBlur="checkDados('ClienteFisico', 'Email', this)" required value="<?php echo($email) ?>">
	</div>
